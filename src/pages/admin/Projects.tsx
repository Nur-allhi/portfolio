...
type Row = { title: string; num: string; desc: string; stack: string[]; repo: string; live: string; status: string };
const KEY = "admin_projects";
const STACK_DEFAULT = ["React", "TypeScript", "Tailwind"];

function load(): Row[] {
  try { const d = JSON.parse(localStorage.getItem(KEY) || ""); if (Array.isArray(d) && d.length) return d; } catch { /* */ }
  return defaults.map(d => ({ title: d.title, num: d.number, desc: d.description, stack: d.stack, repo: d.repoUrl, live: d.liveUrl || "", status: d.status || "completed" }));
}
function save(d: Row[]) { localStorage.setItem(KEY, JSON.stringify(d)); }
...
export function AdminProjects() {
  const [data, setData] = useState<Row[]>(() => load());
  ...
  const onSave = (e: React.FormEvent) => {
    ...
    const obj: Row = { title: title.trim(), num: num.trim(), desc: desc.trim(), stack: tags.slice(), repo: repo.trim() || "#", live: live.trim(), status };
    if (idx >= 0) { const n = [...data]; n[idx] = obj; setData(n); toast("Project updated"); }
    else { setData([...data, obj]); toast("Project added"); }
    setOpen(false);
  };
  const del = (i: number) => { if (!confirm(`Delete "${data[i].title}" — are you sure?`)) return; setData(data.filter((_, j) => j !== i)); toast("Project deleted", true); };
